...
flexplot::visualize(fm1, plot="model", sample=18) +
...
```
But notice that the fixed effects (the black line) is identical between the two models (compare model summaries for fm1 and fm2 to see identical intercepts and slopes for the fixed effects.
Its only the random effects (study participants [Subjects]) that differ.
```{r}
flexplot::model.comparison(fm1, fm2)
#fm1 seems to be better supported
```
